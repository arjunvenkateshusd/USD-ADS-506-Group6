...
data_raw$timestamp <- as.POSIXct(data_raw$timestamp, format="%Y-%m-%d %H:%M:%S", tz="UTC")

data_raw$date <- as.Date(data_raw$timestamp)

daily_caiso <- aggregate(CAISO ~ date, data=data_raw, FUN=mean, na.rm=TRUE)
...
plot(
  ...
  lwd = 1.2,
  xlab = "Date",
  ylab = "Average Load (MW)",
  main = "CAISO Daily Average Electric Load"
)
...
#forecast for the same horizon as ARIMA
fc_tslm <- forecast(fit_tslm, h = length(test_ts))

accuracy(fc_tslm, as.numeric(test_ts))

#plot
autoplot(fc_tslm) +
  autolayer(test_ts, series = "Test Data") +
  labs(
    title = "TSLM Forecast (Trend + Daily Seasonality)",
    x = "Time",
    y = "Load (MW)"
  )
...
accuracy(fc_ets, as.numeric(test_ts))

#plot
autoplot(fc_ets) +
  autolayer(test_ts, series = "Test Data") +
  labs(
    title = "ETS Forecast (Automatic Exponential Smoothing)",
    x = "Time",
    y = "Load (MW)"
  )
...
fit_auto <- auto.arima(train_ts, seasonal = TRUE)
...
autoplot(fc_auto) +
  ...
  ggtitle("ARIMA Forecast (Auto ARIMA)") +
  xlab("Time") +
  ylab("Load (MW)") +
  theme_minimal() +
  theme(
